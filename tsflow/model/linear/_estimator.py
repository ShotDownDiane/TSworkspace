...
class LinearModel:
    ...
    def __call__(self, past_target: np.ndarray, past_observed_values: np.ndarray) -> np.ndarray:
        assert past_target.ndim == 2
        x, scale = self.scaler(past_target, np.ones_like(past_target))
