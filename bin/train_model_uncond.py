...
from tsflow.model import TSFlowUncond
...
def main(
    ...
    assert dataset.metadata.freq == freq.lower()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--logdir", type=str, default="./uncond_logs", help="Path to results dir")
    ...
    # aim_logger.log_hyperparams(config)
    ts_uid = datetime.now(timezone.utc).strftime("%y%m%d-%H%M%S")  # 用UTC避免跨机混乱
    tensorboard_logger = pl.loggers.TensorBoardLogger(
        save_dir=args.logdir,
        name="tsflow_uncond",
        version="tsflow_uncond/"+ts_uid,
    )
    exp_id = "tsflow_uncond_" + ts_uid
    config["logdir"] = args.logdir + "/" + exp_id
    main(**config, loggers=[tensorboard_logger])
